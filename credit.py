...
def add_or_update_copyright(file_path, force_update=False, custom_username=None, custom_github=None):
...
def print_help():
    ...
    help_text = """
# Credit - Copyright Notice Manager
...
```
credit [directory]             # Process files in the specified directory
credit                         # Process files in the default directory
credit --help                  # Show this help message
```

## Configuration

You can configure your username and GitHub handle:

```
credit --config                # View current configuration
credit --setup                 # Set up your configuration
```

## Options

```
-d, --directory DIR            # Specify the directory to process
-f, --force                    # Force update of copyright notices
...
```
credit ./src                   # Process all files in the src directory
credit -l typescript           # Process only TypeScript files in default directory
credit --force                 # Force update all copyright notices
credit --preview ./lib         # Preview changes to files in the lib directory
```

## File Exclusion

Add the comment `// credit-ignore` at the top of any file to exclude it from processing.
"""
...
def preview_changes(files, force_update=False):
    ...
    for file_path in files:
        ...
        if has_copyright and year == str(CURRENT_YEAR) and not force_update:
            ...
            table.add_row(relative_path, f"[blue]Will update ({year} → {CURRENT_YEAR})[/blue]")
        else:
...
Use 'credit --detailed-help' to see complete documentation.
    ...
    parser.add_argument(
        "--setup",
        help="Set up your configuration",
        action="store_true"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--username",
        help="Override username for this run",
        default=None
    )
    parser.add_argument(
        "--github",
        help="Override GitHub handle for this run",
        default=None
    )
    ...
    if args.setup:
        ...
        directory = input(f"Enter default directory [{DEFAULT_DIRECTORY}]: ") or DEFAULT_DIRECTORY
    ...
    if args.file:
        if not os.path.isfile(args.file):
            console.print(f"[bold red]Error:[/bold red] {args.file} is not a valid file.")
            return
    ...
    if platform.system() == "Windows":
        ...
        try:
            ...
            if user_dir not in path:
                ...
                winreg.CloseKey(key)
                console.print("[green]Successfully added to PATH. Please restart your command prompt.[/green]")
            else:
            ...
            console.print("[yellow]You may need to add this directory to your PATH manually.[/yellow]")
    else:
        ...
        try:
            ...
            console.print(f"[yellow]To install, run: sudo cp {script_path} {dest_path} && sudo chmod +x {dest_path}[/yellow]")
        except Exception as e:
